...
def test_load_schema():
    ...
    schema = Schema(uri='./example_schema.yaml', precompile=True)
    schema = Schema(uri='https://gitlab.vm.domaintools.net/external/koalified/raw/master/example_schema.yaml')
    schema = Schema(text=EXAMPLE_SCHEMA, fail_fast=False, score_fields=True, explain=True, precompile=True)
    schema = Schema(uri='./example_schema2.yaml')
